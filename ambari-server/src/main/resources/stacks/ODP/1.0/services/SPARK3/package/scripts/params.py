...
SERVER_ROLE_DIRECTORY_MAP = {
  ...
  'LIVY2_SERVER' : 'livy3-server',
  'LIVY2_CLIENT' : 'livy3-client'
...
if stack_version_formatted and check_stack_feature(StackFeature.SPARK_LIVY2, stack_version_formatted) and "livy3-env" in config['configurations']:
  livy3_component_directory = Script.get_component_from_role(SERVER_ROLE_DIRECTORY_MAP, "LIVY2_SERVER")
  livy3_conf = format("{stack_root}/current/{livy3_component_directory}/conf")
  livy3_log_dir = config['configurations']['livy3-env']['livy3_log_dir']
  livy3_pid_dir = status_params.livy3_pid_dir
  livy3_home = format("{stack_root}/current/{livy3_component_directory}")
  livy3_user = status_params.livy3_user
  livy3_group = status_params.livy3_group
  user_group = status_params.user_group
  livy3_hdfs_user_dir = format("/user/{livy3_user}")
  livy3_server_pid_file = status_params.livy3_server_pid_file
  livy3_recovery_dir = default("/configurations/livy3-conf/livy.server.recovery.state-store.url", "/livy3-recovery")
  livy3_recovery_store = default("/configurations/livy3-conf/livy.server.recovery.state-store", "filesystem")

  livy3_server_start = format("{livy3_home}/bin/livy-server start")
  livy3_server_stop = format("{livy3_home}/bin/livy-server stop")
  livy3_logs_dir = format("{livy3_home}/logs")

  livy3_env_sh = config['configurations']['livy3-env']['content']
  livy3_log4j_properties = config['configurations']['livy3-log4j-properties']['content']
  livy3_spark_blacklist_properties = config['configurations']['livy3-spark-blacklist']['content']

  if 'livy.server.kerberos.keytab' in config['configurations']['livy3-conf']:
    livy_kerberos_keytab =  config['configurations']['livy3-conf']['livy.server.kerberos.keytab']
  else:
    livy_kerberos_keytab =  config['configurations']['livy3-conf']['livy.server.launch.kerberos.keytab']
  if 'livy.server.kerberos.principal' in config['configurations']['livy3-conf']:
    livy_kerberos_principal = config['configurations']['livy3-conf']['livy.server.kerberos.principal']
  else:
    livy_kerberos_principal = config['configurations']['livy3-conf']['livy.server.launch.kerberos.principal']

  livy3_livyserver_hosts = default("/clusterHostInfo/livy3_server_hosts", [])
  livy3_http_scheme = 'https' if 'livy.keystore' in config['configurations']['livy3-conf'] else 'http'
  ...
  if len(livy3_livyserver_hosts) > 0:
    has_livyserver = True
    if security_enabled:
      livy3_principal = livy_kerberos_principal.replace('_HOST', config['agentLevelParams']['hostname'].lower())

  livy3_livyserver_port = default('configurations/livy3-conf/livy.server.port',8999)
